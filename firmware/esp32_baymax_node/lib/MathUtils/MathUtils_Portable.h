...
template<typename T = double>
constexpr T TAU = static_cast<T>(6.28318530717958647692);
...
template<typename T>
inline T sin_pi(T x) {
    return std::sin(x * PI<T>);
}

template<typename T>
inline T cos_pi(T x) {
    return std::cos(x * PI<T>);
}
...
template<typename T>
inline T inSine(T t) {
    return T(1) - cos_pi(t * T(0.5));
}

template<typename T>
inline T outSine(T t) {
    return sin_pi(t * T(0.5));
}

template<typename T>
inline T inOutSine(T t) {
    return T(-0.5) * (cos_pi(t) - T(1));
}
...
inline T inElastic(T t) {
    if (t <= T(0)) return T(0);
    if (t >= T(1)) return T(1);
    return -std::pow(T(2), T(10) * t - T(10)) * sin_pi((t * T(10) - T(10.75)) * T(2) / T(3));
}
...
inline T outElastic(T t) {
    if (t <= T(0)) return T(0);
    if (t >= T(1)) return T(1);
    return std::pow(T(2), T(-10) * t) * sin_pi((t * T(10) - T(0.75)) * T(2) / T(3)) + T(1);
}
...
inline T inOutElastic(T t) {
    ...
    return t < T(0.5)
        ? -(std::pow(T(2), T(20) * t - T(10)) * sin_pi((T(20) * t - T(11.125)) * c)) * T(0.5)
        : (std::pow(T(2), T(-20) * t + T(10)) * sin_pi((T(20) * t - T(11.125)) * c)) * T(0.5) + T(1);
}
...
public:
    ...
    void setThresholds(T high, T low) { m_high = high; m_low = low; }
...
public:
    void push(T val) {
        if (m_count >= WINDOW) {
            m_sum -= m_buffer[m_index];
            m_sumSq -= m_buffer[m_index] * m_buffer[m_index];
        }
